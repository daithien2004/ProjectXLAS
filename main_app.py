...
face_task = st.sidebar.selectbox("Chọn tác vụ nhận dạng:", face_options, key="face_task")
...
# ============ LOGIC XỬ LÝ =============
# Chỉ xử lý nếu một tác vụ được chọn (khác "Chọn tác vụ")
if face_task != "Chọn tác vụ" or chapter3_task != "Chọn tác vụ":
    # Ưu tiên hiển thị Nhận dạng khuôn mặt nếu được chọn
    if face_task != "Chọn tác vụ":
        ...
        if face_task == "Nhận dạng từ Webcam":
        ...
        elif face_task == "Nhận dạng từ video mẫu":
        ...
        elif face_task == "Nhận dạng từ video upload":
    ...
    if chapter3_task != "Chọn tác vụ":
        ...
        if uploaded_image is not None:
            ...
            img = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR if chapter3_task in ["NegativeColor", "HistEqualColor"] else cv2.IMREAD_GRAYSCALE)
            ...
            # Xử lý ảnh
            try:
                processed_img = getattr(chapter3, chapter3_task)(img)
                ...
                with col2:
                    st.image(processed_img, 
                             caption=f"Kết quả ({chapter3_functions[chapter3_task]})", 
                ...
                if st.button("💾 Tải xuống ảnh đã xử lý"):
                    ...
                    st.download_button(
                        ...
                        file_name=f"processed_{chapter3_task}.png",
                        mime="image/png"
                    )
...
# Nếu không chọn tác vụ nào
else:
    st.info("Vui lòng chọn một tác vụ từ menu bên trái để bắt đầu.")