...
def Negative(imgin):
    ...
    # N: độ rộn của ảnh
    M, N = imgin.shape
    # tạo imgout có kích thucows bằng với imgin có màu trangws
...
def NegativeColor(imgin):
    # M: độ cao của ảnh
    # N: độ rộn của ảnh
    # C: số kênh mà, channel laf 3 cho anhr mau
    M, N, C = imgin.shape
    # tạo imgout có kích thucows bằng với imgin có màu trangws
    imgout = np.zeros((M, N, C), np.uint8)
    for x in range(0, M):
        for y in range(0, N):
            # ảnh màu của opencv là RGB
            # ảnh mày của pillow là RGB - pillow của python
            b = imgin[x, y, 0]
            b = L - 1 - b
...
def HistEqualColor(imgin):
    # ảnh mầu của open cv là RGb
    # ảnh màu của pillow là RGB
    # pillow là module zXuwr lí ảnh của python
    b = imgin[:,:,0]
    g = imgin[:,:,1]
    r = imgin[:,:,2]
...
def LocalHist(imgin):
    ...
    for x in range(a, M - a):
        for y in range(b, N - b):
            window = imgin[x-a:x+a+1, y-b:y+b+1]  # Cửa sổ 3x3
            w = cv2.equalizeHist(window)  # Cân bằng histogram trên cửa sổ con
            imgout[x, y] = w[a, b]  # Gán giá trị pixel trung tâm
    return imgout        
...
#     # vanriance = sum/(M*N)
#     # my_stddev = np.sqrt(vanriance)
#     # print(my_mean, my_stddev)
#     mean, stddev = cv2.meanStdDev(imgin)
#     print(mean[0,0], stddev[0,0])
#     return imgout
...
def SharpeningMask(imgin):
    ...
    n=31
    sigma = 5.0
    temp = cv2.GaussianBlur(imgin, (m,n), sigma)
    mask = imgin - temp
    k = 1.0
    imgout = imgin + k*mask
